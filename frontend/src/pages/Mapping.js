...
const Mapping = () => {
  ...
  const fetchMappings = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/mapping/all`);
  ...
  const fetchAvailableFields = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/mapping/fields/available`);
  ...
  const fetchPreviewData = async () => {
    ...
    try {
      const response = await axios.get(`${BASE_URL}/api/mapping/preview`);
  ...
  const handleCreateMapping = async () => {
    try {
      await axios.post(`${BASE_URL}/api/mapping`, newMapping);
      setOpen(false);
      setNewMapping({
        originalField: '',
        customName: '',
  ...
  const handleUpdateMapping = async (mappingId, updatedData) => {
    try {
              await axios.put(`${BASE_URL}/api/mapping/${mappingId}`, updatedData);
      setOpen(false);
  ...
  const handleDeleteMapping = async (mappingId) => {
    try {
              await axios.delete(`${BASE_URL}/api/mapping/${mappingId}`);
      fetchMappings();
    } catch (error) {
  ...
  const handleExport = async (format) => {
    setIsExporting(true);
    try {
      const response = await axios.get(`${BASE_URL}/api/mapping/export/${format}`, {
        responseType: 'blob'
      });
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Autocomplete
            ...
            getOptionLabel={(option) => `${option.field} - ${option.name}`}
            value={availableFields.find(field => field.field === newMapping.originalField) || null}
            onChange={(event, newValue) => {
              setNewMapping({ 
                ...newMapping, 
                originalField: newValue ? newValue.field : '',
                customName: newValue ? newValue.name : newMapping.customName
              });
            }}
            renderInput={(params) => (
              <TextField
                ...
                helperText="Select from available OHW fields"
              />
            )}
                ...
                <Box component="li" key={key} {...otherProps}>
                  <Box>
                    ...
                    <Chip 
                      ...
                      sx={{ mt: 0.5 }}
                      color={
                        option.type === 'uint16' || option.type === 'uint32' || option.type === 'int16' ? 'primary' :
                        option.type === 'string' ? 'secondary' :
                        option.type === 'datetime' ? 'info' :
                        'default'
                      }
                    />
                  </Box>
            ...
            filterOptions={(options, { inputValue }) => {
              ...
              return options.filter(option => 
                option.field.toLowerCase().includes(searchTerm) ||
          ...
          <TextField
            ...
            helperText="Custom name for this field in your system"
          />
