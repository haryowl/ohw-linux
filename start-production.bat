@echo off
title Galileosky Parser - Production Mode
color 0B
...
:: Start frontend server (serving built files)
echo [INFO] Starting frontend server (production mode)...
echo [INFO] Frontend will be available at: http://localhost:3002
echo.

cd ..\frontend
start "Galileosky Frontend" cmd /k "title Galileosky Frontend Server && echo Starting frontend server in production mode... && npm run serve"
