...
echo -e "${BLUE}🚀 Starting Gali-Parse Linux Deployment (Root Version)${NC}"
...
tee /etc/nginx/sites-available/gali-parse << 'EOF'
server {
    listen 80;
    server_name localhost;

    location / {
        root /root/gali-parse-linux/frontend/build;
...
ln -sf /etc/nginx/sites-available/gali-parse /etc/nginx/sites-enabled/
nginx -t
