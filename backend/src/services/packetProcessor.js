...
class PacketProcessor {
    ...
    async processPacket(packet, socket) {
        try {
            ...
            const parsedData = await this.parser.parse(packet);
            if (!parsedData) {
            ...
            if (parsedData.records && parsedData.records.length > 1) {
                ...
                for (let i = 0; i < parsedData.records.length; i++) {
                    ...
                    // Extract IMEI from tag 0x03 or from parser's lastIMEI
                    const imei = record.tags['0x03']?.value || this.parser.lastIMEI || parsedData.imei;
                    
                    if (!imei) {
                        logger.warn(`No IMEI found in record ${i}`, {
                            recordTags: Object.keys(record.tags),
                            parserLastIMEI: this.parser.lastIMEI,
                            parsedDataIMEI: parsedData.imei
                        });
                        continue;
                    }
                ...
                const devicePromises = Array.from(recordsByImei.keys()).map(async (imei) => {
                    ...
                    if (!device) {
                        device = await deviceManager.registerDevice({ imei: imei });
                        logger.info('New device registered:', {
                            imei,
                            timestamp: new Date().toISOString()
                        });
                    } else {
                        await deviceManager.updateDeviceStatus(device.id, 'active');
                    }
                    return { imei, device };
                });
                ...
                // Return the first valid record with device information
                if (validRecords.length > 0) {
                    const firstRecord = validRecords[0];
                    const firstImei = Array.from(recordsByImei.keys())[0];
                    const firstDevice = deviceMap.get(firstImei);
                    ...
                    return {
                        ...
                        mapped: firstRecord.mapped || {},
                        recordNumber: firstRecord.recordNumber,
            ...
            // Extract IMEI from tag 0x03 or from parser's lastIMEI
            const imei = parsedData.records?.[0]?.tags['0x03']?.value || this.parser.lastIMEI || parsedData.imei;
            if (!imei) {
                logger.error('No IMEI found in packet', {
                    ...
                    firstRecordTags: parsedData.records?.[0]?.tags ? Object.keys(parsedData.records[0].tags) : [],
                    parserLastIMEI: this.parser.lastIMEI,
                    parsedDataIMEI: parsedData.imei
                });
                return null;
            ...
            // Return processed data with device information
            return {
                type: parsedData.type,
                deviceId: device.id,
                imei: imei,
                ...
                data: processed.data || {},
                mapped: mapped,
                recordNumber: processed.recordNumber
            };
        } catch (error) {
    ...
    logDeviceParameters(tags, imei) {
        try {
            ...
            if (tags && typeof tags === 'object') {
                for (const [tag, tagData] of Object.entries(tags)) {
                    const value = tagData && tagData.value !== undefined ? tagData.value : tagData;
